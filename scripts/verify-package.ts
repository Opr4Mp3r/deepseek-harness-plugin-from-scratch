...
try {
  ...
  await writeFile(runner, await readFile(resolve(root, 'examples/progressive/tests/loader-runner.mjs'), 'utf8'))
  const { stdout: smokeOutput } = await run(process.execPath, [runner, loaderConfig], {
    cwd: consumerDirectory,
  })
  const resultLine = smokeOutput.split('\n').find(line => line.startsWith('DSH_TUTORIAL_RESULT '))
  if (resultLine === undefined) throw new Error('installed package smoke produced no result')
  const observed = JSON.parse(resultLine.slice('DSH_TUTORIAL_RESULT '.length)) as {
    tools?: string[]
    result?: { isError?: boolean; value?: { message?: string } }
  }
  if (JSON.stringify(observed.tools) !== JSON.stringify(['greet'])
    || observed.result?.isError !== false
    || observed.result.value?.message !== 'Packaged, Ada!') {
    throw new Error(`installed package returned ${JSON.stringify(observed)}`)
  }
} finally {
